...
xdp_fastpath_fn xdp_fastpath_fn_ptr __rcu = NULL;
EXPORT_SYMBOL(xdp_fastpath_fn_ptr);

BPF_CALL_3(bpf_xdp_fastpath, void *, tuple, unsigned char*, dmac, int*, ifindex)
{
    int ret = 0;
    xdp_fastpath_fn func = NULL;

    rcu_read_lock_bh();
    func = rcu_dereference(xdp_fastpath_fn_ptr);
    if (func)
        ret = func(tuple, dmac, ifindex);

    rcu_read_unlock_bh();
    return ret;
}

static const struct bpf_func_proto bpf_xdp_fastpath_proto = {
	.func		= bpf_xdp_fastpath,
	.gpl_only	= false,
    .pkt_access = true,
...
{
	switch (func_id) {
    case BPF_FUNC_xdp_fastpath:
        return &bpf_xdp_fastpath_proto;
	case BPF_FUNC_perf_event_output:
